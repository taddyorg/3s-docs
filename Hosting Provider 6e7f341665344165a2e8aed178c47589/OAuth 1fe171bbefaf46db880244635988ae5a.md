...
Our Test comic, [Larry’s Lullaby](https://taddy.org/feeds/sss/comicseries/96cc49d7-a95d-4266-b408-b57c7d26a62e), is free to read. However, to access the latest 3 episodes of the comic you have to be a Patreon backer of [Daniel Mathews](https://patreon.com/user?u=5547268) ($1 tier, but there is a 7 day free trial). Follow the steps below to be able to view the latest 3 episodes:
...
2. Open a new tab, in your browser and go to [patreon.com](https://www.patreon.com). Log into your Patreon account. Next, go to [Daniel Mathews’](https://patreon.com/user?u=5547268) Patreon account and click ‘Join for free’ to become a free Patreon backer. (This will give you exclusive access to last 3 episodes of Larry’s Lullaby)
...
A link to detailed instructions from the Hosting provider on how to signup for a client_id and client_secret.

## Example Code

Inkverse is an [open-source](https://github.com/taddyorg/inkverse) comic app that has integrated this OAuth workflow in the app to display exclusive content to paying users. Here’s the relevant files:

- Inkverse website opens up a link to Taddy to [get an OAuth cod](https://github.com/taddyorg/inkverse/blob/main/website/app/routes/profile/setup.tsx)e (see getAuthorizationCodeUrl)
- On the server, you [receive the code](https://github.com/taddyorg/inkverse/blob/main/graphql-server/src/routes/hosting-provider.ts) and exchange it for a refresh token and an access token
- On the website, you can [exchange the refresh token](https://github.com/taddyorg/inkverse/blob/main/packages/public/src/hosting-providers.ts) for a new refresh token, a new access token, or a content token for a specific piece of content.
...
{
  "aud": "12", // client_id
  "sub": "789-xyz-123-abc", // client_user_id (user's uuid or id)
